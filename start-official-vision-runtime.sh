...
case "${MAX_NUM_BATCHED_TOKENS:-8192}:${LONG_PREFILL_TOKEN_THRESHOLD:-1024}" in
  *[!0-9:]*|0:*|*:0)
    echo "MAX_NUM_BATCHED_TOKENS and LONG_PREFILL_TOKEN_THRESHOLD must be positive integers" >&2
    exit 2
    ;;
esac
case "${VLLM_PREFIX_CACHE_RETENTION_INTERVAL:-4096}" in
  ''|*[!0-9]*)
    echo "VLLM_PREFIX_CACHE_RETENTION_INTERVAL must be an integer" >&2
    exit 2
    ;;
esac
if (( ${VLLM_PREFIX_CACHE_RETENTION_INTERVAL:-4096} % 256 != 0 )); then
  echo "VLLM_PREFIX_CACHE_RETENTION_INTERVAL must be a multiple of the 256-token block size" >&2
  exit 2
fi
...
worker_env="NODE_RANK=1 HEADLESS=1 VLLM_HOST_IP=$WORKER_VLLM_HOST_IP OFFICIAL_VISION_IMAGE=$OFFICIAL_VISION_IMAGE DSPARK_MODEL_OFFICIAL=$DSPARK_MODEL_OFFICIAL DSPARK_REVISION=$DSPARK_REVISION OFFICIAL_MAX_MODEL_LEN=$OFFICIAL_MAX_MODEL_LEN OFFICIAL_GPU_MEMORY_UTILIZATION=$OFFICIAL_GPU_MEMORY_UTILIZATION OFFICIAL_MTP_NUM_TOKENS=$OFFICIAL_MTP_NUM_TOKENS OFFICIAL_ADAPTIVE_VERIFICATION=${OFFICIAL_ADAPTIVE_VERIFICATION:-false} VLLM_ADAPTIVE_VERIFICATION_PROFILE_CONTEXT_LEN=${VLLM_ADAPTIVE_VERIFICATION_PROFILE_CONTEXT_LEN:-131072} OFFICIAL_RUNTIME_CACHE_ROOT=${OFFICIAL_RUNTIME_CACHE_ROOT:-/cache/huggingface/runtime-cache/vllm-1356635-pr54631-sm121} VLLM_USE_BREAKABLE_CUDAGRAPH=${VLLM_USE_BREAKABLE_CUDAGRAPH:-1} VLLM_PREFIX_CACHE_RETENTION_INTERVAL=${VLLM_PREFIX_CACHE_RETENTION_INTERVAL:-4096} MAX_NUM_BATCHED_TOKENS=${MAX_NUM_BATCHED_TOKENS:-8192} LONG_PREFILL_TOKEN_THRESHOLD=${LONG_PREFILL_TOKEN_THRESHOLD:-1024} DSPARK_RESTART_POLICY=${DSPARK_RESTART_POLICY:-no}"
...
run_boot_warmup() {
  ...
  echo "Running text and vision kernel warmup before accepting traffic..."
  DSPARK_WARMUP_BATCH_TOKENS="${MAX_NUM_BATCHED_TOKENS:-8192}" \
    DSPARK_WARMUP_SPEC_TOKENS="${OFFICIAL_MTP_NUM_TOKENS:-3}" \
    DSPARK_WARMUP_VISION=1 \
    bash "$SCRIPT_DIR/scripts/boot-shape-warmup.sh" \
      "http://127.0.0.1:${VLLM_PORT:-8888}" \
      "${SERVED_MODEL_NAME:-deepseek-v4-flash-vision-exp}" ||
    echo "WARN: boot shape warmup incomplete; uncovered kernels may JIT during serving" >&2
}
